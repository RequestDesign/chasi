...
$(function () {
  ...
  function resetLoginFormFields() {
    $(
      "#emailLoginInput, #phoneLoginInput, #emailLoginPasswordInput, #phoneLoginPasswordInput"
    ).val("");
  ...
  function showEmailLoginForm() {
    ...
    $(".switcher--auth")
      ...
      .filter('[data-switcher="EmailLogin"]')
  ...
  $(".switcher--auth").on("click", function () {
    ...
    if (!$this.hasClass("switcher--active")) {
      ...
      $(".switcher-content")
        .hide()
        .filter(`[data-inputs='${switcherType}']`)
        .show();
    }
  });
  ...
  $(".input-eye input").on("change", function (evt) {
    if ($(this).is(":checked")) {
      $(this).closest(".input-wrapper").find("input[type=password]")[0].type =
        "text";
    } else {
      $(this).closest(".input-wrapper").find("input[type=text]")[0].type =
        "password";
    }
  });
});
